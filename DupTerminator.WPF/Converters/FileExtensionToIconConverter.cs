...
namespace DupTerminator.WPF.Converters
{
    public class FileExtensionToIconConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (value is ExtendedFileInfoViewModel efim)
            {
                return IconCache.Instance.GetIcon(efim.Extension);
            }
            else if (value is ExtendedFileInfo efi)
            {
                if (efi is DirectoryContainer)
