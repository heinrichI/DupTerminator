                    ...
                    if (pair.Value.Count > 1)
                    {
                        ...
                        for (int i = 0; i < files.Count; i++)
                        {
                            for (int j = i + 1; j < files.Count; j++)
                            {
                                ...
                                value.Item1.Add(first);
                                value.Item2.Add(second);
                            }
                        }
                    }
                }
            ...
            in string phisicalDrive)
        {
            ...
            foreach (var item in files3)
            {
                ...
                if (item.Container is null)
                    item.Container = new ExtendedFileInfo { Path = di.FullName };
                files.Add(item);
                if (_archiveService.IsArchiveFile(item.Path))
                {
                    var filesInArchive = _dbArchiveService.Get(_searchSetting.UseDB, item, token).ToList();
                        //.Select(f => new ContainerFileInfo(f)).ToList();
                    ...
                    {
                        file.ContainerFilesCount = filesInArchive.Count;
                        files.Add(file);
                    }
                }
            }
